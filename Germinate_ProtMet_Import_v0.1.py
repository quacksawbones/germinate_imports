...
class Datasets(Base):
    ...
    location_id = Column(Integer) #NB: Foreign Key to locations - Just use an integer, no need to have the foreign key proper
    description = Column(String) #NB: This needs to be UNIQUE AND DESCRIPTIVE. This is what is displayed when the data sets are extracted from Germinate
    date_start = Column(String)
    date_end = Column(String)
    datatype = Column(String) #NB: Probably need a set vocabulary, or at least a word or two to describe it...
    created_by = Column(String)
    dataset_state_id = Column(Integer) #NB A single digit to describe who can see the data. 1 is everyone who can access Germinate, 2 is the creator and the admins, 3 is admin only. Use 1
    #license_id = Column(Integer) #NB: This needs to be investigated further. At the moment, use 1
    created_on = Column(String)
    contact = Column(String) #NB: Make sure it is an e-mail address of someone that can talk about the data!
...
if any([createProtsAA,createProtsBin,loadProtAAData,loadProtBinData]):
    ...
    if any([createProtsBin,loadProtBinData]):
        ...
        if loadProtBinData:
            ...
            for funcbinsample in functional_parsed.columns:
                ...
                if funcbinsample[:4] != "IW_S":
                    ...
                    for i in range(1,len(temp_functbin)):
                        ...
                        value_for_insertion = None

                        if temp_functbin.iat[i,1] != temp_functbin.iat[i,1]:
                            continue
                        else:
                            value_for_insertion = temp_functbin.iat[i,1]
                                

                        functbinload = CompoundData(compound_id=existing_protbin_result,germinatebase_id=acc_existing_result,dataset_id=1,compound_value=value_for_insertion,created_on=datetime.datetime.now())                        
            ...
            print ('%d functional proteomic bin values added to Germinate') % len(session.new)
            session.commit()     
